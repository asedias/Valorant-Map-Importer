...
def is_importable(object):
    ...
    if "Properties" in object:
        ...
        if any(x == object["Type"] for x in importable_types):
...
def get_object_name(objectProperties):
    p = Path(objectProperties["StaticMesh"]["ObjectPath"]).stem
    return p


def get_object_path(objectProperties):
    return objectProperties["StaticMesh"]["ObjectPath"]


def get_fixed_path(objectProperties):
    object_path = objectProperties["StaticMesh"]["ObjectPath"].replace("ShooterGame/Content", "Game")
    a = CWD.joinpath("export", os.path.splitext(object_path)[0].strip("/")).__str__()
    return a
...
def get_mat_path_og(mat: dict):
    return mat["OuterIndex"]["ObjectName"]

    
def get_mat_path(mat: dict):
    mat_path = mat["ObjectPath"].replace("ShooterGame/Content", "/Game").split(".")[0]
    return mat_path
    ...
    if "ScalarParameterValues" in matJSON:
        for param in matJSON["ScalarParameterValues"]:
            if param["ParameterInfo"]["Name"] == "Mask Blend Power":
                ...
                if _DEBUG:
                    logger.warning(f"{byoMAT.name}: Found an unset ScalarParameterValue: {param['ParameterInfo']['Name']}")
        ...
        for texPROP in matJSON["TextureParameterValues"]:
            ...
            if Path(texPath).exists():
                ...
                if texPROP["ParameterInfo"]["Name"] == "Diffuse":
                    ...
                    if _DEBUG:
                        logger.warning(f"{byoMAT.name}: Found an unset TextureParameterValue: {param['ParameterInfo']['Name']}")
    ...
    if "VectorParameterValues" in matJSON:
        for param in matJSON["VectorParameterValues"]:
            if param["ParameterInfo"]["Name"] == "DiffuseColor":
                ...
                # pass
                if _DEBUG:
                    logger.warning(f"{byoMAT.name}: Found an unset VectorParameterValue: {param['ParameterInfo']['Name']}")
        ...
        for index, mat in enumerate(objectProperties_OG["StaticMaterials"]):
            if type(mat["MaterialInterface"]) is dict:
                ...
                if "WorldGridMaterial" not in matName:
                    matPath = get_mat_path_og(mat["MaterialInterface"])
...
def resetProps(object):
    object.scale = [1, 1, 1]
    object.rotation_euler = [0, 0, 0]

def objectSetSMData(imported, objectSMData, props):
    scaleX = 1
    scaleY = 1
    scaleZ = 1
    if "RelativeScale3D" in props:
        scaleX = int(props["RelativeScale3D"]["X"])
        scaleY = int(props["RelativeScale3D"]["Y"])
        scaleZ = int(props["RelativeScale3D"]["Z"])
        if _DEBUG:
            logger.info(f"PP Scale XYZ: {scaleX} {scaleY} {scaleZ}")
    if "RelativeScale3D" in objectSMData:
        SCX = objectSMData["RelativeScale3D"]["X"]
        SCY = objectSMData["RelativeScale3D"]["Y"]
        SCZ = objectSMData["RelativeScale3D"]["Z"]
        imported.scale = [
            SCX * scaleX,
            SCY * scaleY,
            SCZ * scaleZ,
        ...
        if _DEBUG:
            logger.debug(f"SM Scale XYZ: {SCX} {SCY} {SCZ}")
    if "OffsetLocation" in objectSMData:
        offsetloc = objectSMData["OffsetLocation"]
        offsetloc["X"] *= 0.01
        offsetloc["Y"] *= 0.01
        offsetloc["Z"] *= 0.01
        imported.location = [
            imported.location[0] + (offsetloc["X"] * scaleX),
            imported.location[1] - (offsetloc["Y"] * scaleY),
            imported.location[2] + (offsetloc["Z"] * scaleZ)
        ]
    if "RelativeRotation" in objectSMData:
        rotation = objectSMData["RelativeRotation"]
        imported.rotation_mode = 'XYZ'
        imported.rotation_euler = [
            radians(rotation["Roll"]),
            radians(-rotation["Pitch"]),
            radians(-rotation["Yaw"]*sign(scaleY)) #???
        ]
        roll = rotation["Roll"]
        pitch = rotation["Pitch"]
        yaw = rotation["Yaw"]
        if _DEBUG:
            logger.debug(f"Rotation XYZ: {roll} {pitch} {yaw}")
...
def import_object(object, objectIndex, umap_name, mainScene):
    ...
    objCheck = bpy.data.meshes.get(objName)
    crt_JSON_FOLDER = JSON_FOLDER.joinpath(SELECTED_MAP, umap_name, objName)

    if _SAVE_JSONS:
        crt_JSON_FOLDER.mkdir(parents=True, exist_ok=True)
        save_json(p=crt_JSON_FOLDER.joinpath(object["Outer"] + ".json"), d=object)
    ...
    if objCheck is None:
        if Path(objPath).exists():
            ...
            if _USE_OUTER_NAMES:
                objName = object["Outer"]
                imported.name = object["Outer"]
                logger.info(f"[{objectIndex}] : Rename : {imported.name}")
            ...
            if _SAVE_JSONS:
                save_json(p=crt_JSON_FOLDER.joinpath(object["Outer"] + "_OG" + ".json"), d=objJSON_OG)
            ...
            if "PerInstanceSMData" in object:
                objectSMData = object["PerInstanceSMData"]
                objectSetSMData(imported, objectSMData[0], objectProperties)
                if len(objectSMData) > 1:
                    if _DEBUG:
                        logger.info(f"Reference count: {len(objectSMData)}")
                    for SMData in objectSMData[1:]:
                        #new_imported = imported.copy()
                        new_imported = bpy.data.objects.new(objName, imported.data)

                        blenderUtils.objectSetProperties(new_imported, objectProperties)
                        objectSetSMData(new_imported, SMData, objectProperties)
                        bpy.data.collections[umap_name].objects.link(new_imported)    
        ...
        new_obj = bpy.data.objects.new(objName, objCheck)

        if _USE_OUTER_NAMES:
            objName = object["Outer"]
            new_obj.name = object["Outer"]
            logger.info(f"[{objectIndex}] : Rename : {new_obj.name}")
        resetProps(new_obj)
        ...
        if "PerInstanceSMData" in object:
                objectSMData = object["PerInstanceSMData"]
                objectSetSMData(new_obj, objectSMData[0], objectProperties)
                if len(objectSMData) > 1:
                    if _DEBUG:
                        logger.info(f"Reference count: {len(objectSMData)}")
                    for SMData in objectSMData[1:]:
                        #new_imported = new_obj.copy()

                        new_imported = bpy.data.objects.new(objName, new_obj.data)
...
@timer
def import_json(data, umap_name: str, map_type: str):
    
    os.system("cls")
    title = f"\rImporting {SELECTED_MAP.capitalize()}: Processing {umap_name}"
    ...
    if map_type is "vfx" or map_type is "object":
        ...
        if map_type is "object":
            objectsToImport = filter_objects(data)
    ...
    if not _DEBUG:
        CWD.joinpath("export", "Scenes", SELECTED_MAP.capitalize()).mkdir(parents=True, exist_ok=True)
        bpy.ops.wm.save_as_mainfile(filepath=CWD.joinpath("export", "Scenes", SELECTED_MAP.capitalize(), umap_name).__str__() + ".blend", compress=True)


def combine_umaps():

    # ! Import other .blend files back!
    blend_folder = CWD.joinpath("export", "Scenes", SELECTED_MAP)
    blend_paths = Path(blend_folder).glob('*.blend')
    for umap_blend in blend_paths:
        #umap_name = os.path.splitext(os.path.basename(umap))[0]
        #umap_blend_file = CWD.joinpath("export", "Scenes", SELECTED_MAP.capitalize(), umap_name).__str__() + ".blend"
        umap_name = os.path.basename(umap_blend).split(".")[0]
        umap_blend_file = umap_blend.__str__()
...
def main():
    ...
    # --------------------------------------------------
    # Blender Loop
    MAP_FOLDER = CWD.joinpath("export", "Data", SELECTED_MAP.capitalize())
    data_paths = Path(MAP_FOLDER).glob('*.json')
    for path in data_paths:
        with open(path, "r") as rawdata:
            data = json.load(rawdata)
            umap_name = os.path.basename(path).split(".")[0]
            if not _DEBUG:
                blenderUtils.cleanUP()
            if "Lighting" in umap_name:
                if _IMPORT_LIGHTS:
                    import_json(data, umap_name=umap_name, map_type="lights")
            elif "VFX" in umap_name:
                import_json(data, umap_name=umap_name, map_type="vfx")
            else:
                import_json(data, umap_name=umap_name, map_type="object") 
